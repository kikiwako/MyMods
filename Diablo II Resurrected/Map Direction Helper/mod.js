const levelsFile = "/data/local/lng/strings/levels.json";
...
const levelsData = D2RMM.readJson(levelsFile);
applyChanges(levelsData);
D2RMM.writeJson(levelsFile, levelsData);

function applyChanges(data) {
    const appendString = (key, ...strings) => {
        let i = -1;
        data.find((string, index) => {
            if (string.Key === key) i = index;
            return string.Key === key;
        });

        if (i === -1) {
            console.error("Level string with key " + key + " not found");
        } else {
            data[i].enUS += strings.map(str => `\n${str}`).join('');
        }
    };
    ...
    appendString("Rigid Highlands", `N-E ${or} N-W ${to} Arreat Plateau${location}`, `${follow}Stairs`);
    appendString("Arreat Plateau", `N-E ${or} N-W ${to} Crystalline Passage${location}`, `${follow}Stairs`);
    appendString("Crystalized Cavern Level 1", `${right} ${to} ${waypoint}${location}`, `${up} ${to} Frozen River${location}`, `${left} ${to} Glacial Trail${location}`);
    ...
    appendString("Tundra Wastelands", `N-W ${to} Ancients${location}`);
